...
pub struct FormatParams {
    /// MPEG4-GENERIC AudioSpecificConfig.
    pub aac_config: Option<u16>,
    /// RFC 3640 AAC-hbr mode.
    pub aac_hbr: Option<bool>,
    /// AU size field width in bits.
    pub aac_size_length: Option<u8>,
    /// AU index field width in bits.
    pub aac_index_length: Option<u8>,
    /// AU index delta field width in bits.
    pub aac_index_delta_length: Option<u8>,
...
impl FormatParams {
    ...
    pub(crate) fn set_param(&mut self, param: &FormatParam) {
        use FormatParam::*;
        match param {
            AacConfig(v) => self.aac_config = Some(*v),
            AacHbr(v) => self.aac_hbr = Some(*v),
            AacSizeLength(v) => self.aac_size_length = Some(*v),
            AacIndexLength(v) => self.aac_index_length = Some(*v),
            AacIndexDeltaLength(v) => self.aac_index_delta_length = Some(*v),
    ...
    pub(crate) fn to_format_param(self) -> Vec<FormatParam> {
        ...
        if let Some(v) = self.aac_config {
            r.push(AacConfig(v));
        }
        if let Some(v) = self.aac_hbr {
            r.push(AacHbr(v));
        }
        if let Some(v) = self.aac_size_length {
            r.push(AacSizeLength(v));
        }
        if let Some(v) = self.aac_index_length {
            r.push(AacIndexLength(v));
        }
        if let Some(v) = self.aac_index_delta_length {
            r.push(AacIndexDeltaLength(v));
        }
