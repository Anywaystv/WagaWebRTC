...
impl FormatParam {
    pub fn parse(k: &str, v: &str) -> Self {
        use FormatParam::*;
        match k {
            "config" => u16::from_str_radix(v, 16).map(AacConfig).ok(),
            "mode" => Some(AacHbr(v.eq_ignore_ascii_case("AAC-hbr"))),
            "sizelength" | "sizeLength" => v.parse().map(AacSizeLength).ok(),
            "indexlength" | "indexLength" => v.parse().map(AacIndexLength).ok(),
            "indexdeltalength" | "indexDeltaLength" => v.parse().map(AacIndexDeltaLength).ok(),
...
impl fmt::Display for FormatParam {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        use FormatParam::*;
        match self {
            AacConfig(v) => write!(f, "config={v:04x};streamtype=5"),
            AacHbr(v) => write!(f, "mode={}", if *v { "AAC-hbr" } else { "unsupported" }),
            AacSizeLength(v) => write!(f, "sizeLength={v}"),
            AacIndexLength(v) => write!(f, "indexLength={v}"),
            AacIndexDeltaLength(v) => write!(f, "indexDeltaLength={v}"),
